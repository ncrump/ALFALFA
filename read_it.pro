...
;of their data in order to box and save the good stuff. This program reads the data file created by  
;'box_it.pro' (given its filename by the user at the end of that program session) to serve two functions based 
;on the user's input OPTION at runtime. For 'OPTION' = 1: this program plots each channel of data in 'newarr' 
;one channel at a time in order to see the saved data in the array. In this option, a LEFT mouse click proceeds  
;to the next channel; a RIGHT mouse click ends the session. For 'OPTION' = 0: this program creates 
;and plots the array sum called 'sumarr' which is a 2-dimensional [144x144] array containing only the x,y values 
;summed over all of the channels in the original array ('newarr'). At the end of this session you will be given 
;the option to save the data that was created and assign a file name. Your saved file will contain the following 
;variables:
...
sumarr=dblarr(size_x,size_y)
  
;loop through the channel range to create sum array
for i = 0, chann_range-1 do begin
    sumarr = sumarr + newarr[i,0,*,*]
endfor
